...
6. Perform a longer recording on the target device and observe memory usage, device heating, frame rate, and errors when storage is nearly full. In automatic mode, specifically test the process-memory query and saving when the available-memory reserve falls below 256 MiB. This memory-monitoring behaviour has not been tested here because suitable Apple LiDAR hardware was unavailable. This build makes no performance or accuracy claim validated on physical hardware.

The successful core tests and Simulator build do not replace testing on physical LiDAR hardware. Build 2 must therefore be regarded as an implemented, partially validated project state.
